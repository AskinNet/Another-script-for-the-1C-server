...
// all          - блокировка соединений, перезагрузка сервера и обновления баз из хранилищ
...
Функция ПодключитьсяКластеру(База)
...
Процедура БлокировкаНаБазы(Знач ТекущийКластер, Знач СписокБаз, Установить = Ложь)
...
Функция ЗапуститьRAS()
	...
	Порт = "--port=" + МенеджерПараметров.Параметр("serverRAS.mportRAS") 
					 ...
					 + ":" + МенеджерПараметров.Параметр("serverRAS.port");
...
//ТекущийКластер = ПодключитьсяКластеру(МассивОбслуживаемыхБаз[0]);
...
ИначеЕсли КомандаСкрипта = "lock" Тогда
	//БлокировкаНаБазы(ТекущийКластер, МассивОбслуживаемыхБаз, Истина);
ИначеЕсли КомандаСкрипта = "unlock" Тогда
	//БлокировкаНаБазы(ТекущийКластер, МассивОбслуживаемыхБаз, Ложь);
ИначеЕсли КомандаСкрипта = "restart" Тогда
	ПерезапускСервера();
КонецЕсли;

Если КомандаСкрипта = "all" Тогда
	//БлокировкаНаБазы(ТекущийКластер, МассивОбслуживаемыхБаз, Истина);
	ПерезапускСервера();
	Для Каждого База Из МассивОбслуживаемыхБаз Цикл
