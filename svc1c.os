...
// allstages       			- блокировка соединений, перезагрузка сервера и обновления баз из хранилищ
...
Функция ИнициализацияОкружения()
	...
	Если АргументыКоманднойСтроки.Количество() > 0 Тогда
		...
		Если АргументыКоманднойСтроки.Количество() > 2 Тогда
			КомандаОтладки = АргументыКоманднойСтроки[2];
			Если КомандаОтладки = "" Тогда
				РежимОтладки = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Если КомандаСкрипта = "allstages" Тогда
	...
	КонецЦикла;		
	БлокировкаНаБазы(МассивОбслуживаемыхБаз, Истина);
	Если РестарСервераДоОбновления Тогда
		ПерезапускСервера();
	КонецЕсли;
	Для Каждого База Из МассивОбслуживаемыхБаз Цикл
		ОбновитьКонфигурациюБазы(База);
	КонецЦикла;
	Если НЕ РестарСервераДоОбновления Тогда
		ПерезапускСервера();
	КонецЕсли;		
	БлокировкаНаБазы(МассивОбслуживаемыхБаз, Ложь);
КонецЕсли;
